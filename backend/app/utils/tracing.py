...
def redact_secrets(payload: Any) -> Any:
    ...
    if isinstance(payload, dict):
        ...
        for k, v in payload.items():
            k_lower = str(k).lower()
            if any(secret_term in k_lower for secret_term in ["secret", "api_key", "authorization", "cookie", "password", "token", "rzp_test", "rzp_live"]):
                redacted[k] = "[REDACTED]"
            else:
        ...
        # Redact obvious token patterns in strings
...
def hash_trace_event(trace_id: str, sequence_no: int, timestamp: str, event_type: str, payload: Dict[str, Any], previous_event_hash: str) -> str:
